# Автоматическая сборка и развертывание проекта «в один клик»

Ниже представлен универсальный автоматизированный `bash`-скрипт. Он самостоятельно инициализирует конфигурацию Go Workspaces (`go.work`), подтягивает модули из локальных директорий, генерирует криптографические TLS-сертификаты для QUIC-сервера, запускает тесты ядра и компилирует все бинарники в папку `bin/`.

## 🚀 Скрипт автосборки `setup.sh`

Создайте файл `scripts/setup.sh` (или запустите этот код прямо из корня проекта):
...
После завершения скрипта вы можете запускать скомпилированные оптимизированные бинарники напрямую из папки `bin/` (например, `./bin/udp-server` и `./bin/udp-client`), либо пользоваться шорткатами `Makefile`.
